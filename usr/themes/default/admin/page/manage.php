<?php $v->layout('admin/layout/admin.layout.php', ['title' => '管理页面']) ?>
            ...
            <div class="table-responsive">
                <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
                    <div class="dataTables_length" id="DataTables_Table_0_length">
                        <a class="btn btn-primary ml-auto" href="<?= $v->url("admin/page/write"); ?>">
                            新建页面
                        </a>
                    </div>
                    ...
                    <table class="table card-table table-vcenter text-nowrap datatable dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                        ...
                        <tbody>
                            ...
                            <?php foreach ($pages as $page) : ?>
                                ...
                                <tr data-content-id="<?= $page->id; ?>" role="row" class="<?= $odd = !$odd ? 'odd' : 'even'; ?>">
                                    <td><span class="text-muted"><?= $page->id; ?></span></td>
                                    <td>
                                        <a target="_blank" href="<?= $page->url; ?>" class="text-inherit"><?= $page->title; ?></a>
                                        <a href="<?= $v->url('admin/page/write/' . $page->id); ?>"><i class="fe fe-edit"></i></a>


                                    </td>
                                    <td><?= $page->author['screenName']; ?></td>
                                    <td><?= get_category($page) ?></td>
                                    <td><?= $page->date() ?></td>
                                    <td><?= $page->dateUpdated() ?></td>
                                    <td><?= get_status_html($page->status); ?></td>
                                    <td class="text-right">
                    ...
                    <div class="dataTables_paginate paging_simple_numbers" id="DataTables_Table_0_paginate">
                        ...
                        <a class="paginate_button previous <?= $prev["enabled"] ? '' : 'disabled'; ?>" <?php if ($prev["enabled"]) : ?> href="<?= $v->url('admin/page/manage/' . $prev['url']); ?>" <?php endif; ?> data-dt-idx="0" tabindex="0" id="DataTables_Table_0_previous">
                            上一页
                        </a>
                        <span>
                            <?php foreach ($pageNav['inter'] as $page) : ?>
                                <?php if ($page['type'] == 'page') : ?>
                                    <a class="paginate_button <?= $page["isCurrent"] ? 'current' : ''; ?>" href="<?= $v->url('admin/page/manage/' . $page['url']); ?>" data-dt-idx="1" tabindex="0"><?= $page['page']; ?></a>
                                <?php elseif ($page['type'] == 'hellip') : ?>
                                    <span>...</span>
                                <?php endif; ?>
                            <?php endforeach; ?>
                        </span>
                        <a class="paginate_button next  <?= $next["enabled"] ? '' : 'disabled'; ?>" <?php if ($next["enabled"]) : ?> href="<?= $v->url('admin/page/manage/' . $next['url']); ?>" <?php endif; ?> data-dt-idx="3" tabindex="0" id="DataTables_Table_0_next">
                            下一页
                        </a>
                    </div>
                </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(".menuitem-edit").click(($evt) => {
            var id = getContentId($evt);
            window.location.href = "<?= $v->url("admin/page/write"); ?>/" + id;
        });
